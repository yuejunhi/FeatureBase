          ...
          <el-form :model="item" ref="form" label-width="100px">
            <el-form-item label="上传文件">
              ...
              <el-upload v-if="item.percentFlag == false"
                action="http://172.16.63.109:6868/api/AdditionalConstruction/UploadFile" list-type="picture-card"
  ...
  export default {
    ...
    methods: {
      ...
      setParams(isUpdate) { // 设置参数
        var params = {
          AdditionalId: this.tableRowData.Id,
          BasicParaDataObj: this.changeSegContent(JSON.parse(JSON.stringify(this.currentPara)), 1),
          AboveParaList: this.changeSegContent(JSON.parse(JSON.stringify(this.prevItems)).reverse(), 2),
          BelowParaList: this.changeSegContent(JSON.parse(JSON.stringify(this.nextItems)), 2),
        ...
        if (type == 1) {
          ...
          if (objData.length > 0) {
            for (var index = objData.length - 1; index >= 0; index--) {
              ...
              if (dataObj.Keyword.trim().length == 0 && dataObj.PosFeature.trim().length == 0 && dataObj.NegFeature.trim().length ==
                0) {
      ...
      setDataObj(conditionDataObj) {
        ...
        if (conditionDataObj.AdditionalId && conditionDataObj.ConditionId) {
        ...
        this.formData.AdditionalId = this.tableRowData.Id;
        // console.log(this.conditionData)
        // if(this.conditionData.AdditionalId){
        //   this.isShowUpdateBtn = true;
      ...
      submitData() {
        ...
        this.$axios({
          method: 'post', //请求的方式
          url: '/api/AdditionalConstruction/AddCondition',
          data: params
        }).then(res => {
          console.log(res);
          loading.close();
          if (res.status == 200 && res.data != null && res.data.Status == 1) {
            ...
            this.conditionData.AdditionalId = res.data.Data.AdditionalId;
      ...
      updateData() {
        ...
        this.$axios({
          method: 'put', //请求的方式
          url: '/api/AdditionalConstruction/UpdateCondition',
          data: params
        }).then(res => {
          console.log(res);
          loading.close();
