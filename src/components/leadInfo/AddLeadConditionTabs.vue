<template>
  <div class="add-contion-tabs">
    ...
    <div v-if="editableTabs.length == 0" style="text-align: center;height: 60px;line-height: 60px;">该导语暂时没有段落特征</div>
  </div>
</template>

<script>
  import AddLeadCondition from './AddLeadCondition'
   export default {
     name:'AddLeadConditionTabs',
     components:{AddLeadCondition},
     props:['tableRowData','allConditionsInfo'],
      ...
      methods: {
        ...
        handleTabsEdit(targetName, action) {
          ...
          if (action === 'add') {
            if(this.editableTabs.length == 0 || this.editableTabs[this.editableTabs.length - 1].LeadId){
              let newTabName = ++this.tabIndex + '';
              let title = (this.editableTabs.length+1) + '';
              this.editableTabs.push({
                title: '段落特征'+title,
                name: newTabName,
                content: 'AddLeadCondition'
              });
          ...
          if (action === 'remove') {
            this.$confirm('确认删除该段落特征？')
              .then(_ => {
                ...
                if(optIndex > -1 && tabs[optIndex].LeadId && tabs[optIndex].LeadId.length > 0 &&
        ...
        setData(){
          ...
          for(var index = 0;index < this.editableTabs.length;index++){
            ...
            editableTab.content = 'AddLeadCondition';
          }
        ...
        getConditionData(dataObj){
          ...
          if(clickIndex > -1){
            ...
            editableTab.LeadId = dataObj.LeadId;
        ...
        delCondition(dataObj,targetName,tabs) {  // 删除段落特征
          ...
          this.$axios({
            method:'delete',  //请求的方式
            url:'/api/LeadConstruction/DeleteCondition',
